...
export class ThemeService implements OnDestroy {
  ...
  private toggleClassOnThemeChanges(): void {
    ...
    this.theme$.pipe(takeUntil(this._destroyed$)).subscribe((theme) => {
      ...
      if (theme === 'dark') {
        this._renderer.addClass(this._document.documentElement, 'tw-dark');
      } else {
        // else if is added already, we remove it
        if (this._document.documentElement.className.includes('tw-dark')) {
          this._renderer.removeClass(this._document.documentElement, 'tw-dark');
        }
      }
    });
  }
